...
- Refresh the README recording around managed Host/ProxyJump routing and add
  ...
  compatibility.
- Move the `host`, `auth`, local toolbox lifecycle, and remote file transfer
  command implementations out of the binary entry point into focused modules
  without changing their CLI interfaces.
